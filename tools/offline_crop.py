...
def parse_bbox(bbox_str):
    try:
        if not isinstance(bbox_str, str) or bbox_str == '[]' or bbox_str.strip() == '':
            return None
...
def offline_crop(csv_path, images_dir, output_dir, target_size=512, margin=0.1, skip_empty=False):
    df = pd.read_csv(csv_path)
    images_dir = Path(images_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"Cropping images from {csv_path} to {output_dir}...")
    print(f"Policy: skip_empty={skip_empty}")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        # Policy: if no bbox found and skip_empty is True (for fin training), do not save
        if bbox is None and skip_empty:
            count_skipped += 1
            continue
        ...
        if bbox is not None:
            ...
            crop_img = img # No bbox and not skip_empty (e.g. FB or test)
    ...
    print(f"Done! Saved: {count_ok}, Failed: {count_fail}, Skipped (no fin): {count_skipped}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--phase", type=str, default="train", choices=["train", "test"])
    parser.add_argument("--type", type=str, default="backfin", choices=["backfin", "fb"])
    parser.add_argument("--size", type=int, default=512)
    args = parser.parse_args()

    if args.type == "backfin":
        # Use refined results
        csv_train = "happywhale_data/train_backfin_refined.csv"
        csv_test = "happywhale_data/test_backfin_refined.csv"
        out_prefix = "backfin"
        # For backfin training, we ONLY want images with detected fins
        skip_empty = (args.phase == "train") 
    else:
        # For fullbody, use the best available bbox
